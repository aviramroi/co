...
### Input cost — paid on _every_ turn
...
Don't ship even the facade's full detail up front. Mirror the pattern _this very
harness_ uses with its deferred tools:
...
> **TIF (`format: "tif"`).** The shaper also emits
> [TIF — Token Interchange Format](../src/tif/README.md): schema-once +
> positional rows + dictionary compression for repeated strings. Measured with
> the **real** OpenAI (`js-tiktoken`) and Claude (`@anthropic-ai/tokenizer`)
> tokenizers on 50-row API-like payloads, TIF is **~50–55% smaller than compact
> JSON** (and beats CSV/YAML/XML) while remaining **losslessly decodable** — the
> LLM reads TIF, the proxy still sends the backend real JSON. See
> `src/tif/benchmark.test.ts` for the numbers and `src/tif/` for the codec.
...
- Default `describe` returns only `{name, type, required, refTo}` per field and
  a **picklist value _count_**, not the values. `detail: "full"` opts back in.
...
export function shapeQuery(
  raw: { records: SfRecord[]; totalSize: number; done: boolean },
  opts: ShapeOpts = {},
) {
